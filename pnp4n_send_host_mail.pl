#!/usr/bin/perl
#
# Test: perl pnp4n_send_host_mail.pl -r my_email@domain.com -f html -t
#
# ############################ pnp4n_send_host_mail.pl ################ #
# Date    : 2015-11-20                                                  #
# Purpose : Script to send out Icinga2 host e-mails                     #
...
# language translated 
# ########################################################################
require "$Bin/lang/host.languages.config.pm";
...
# ########################################################################
sub help {
   print "\nIcinga2 e-mail notification script for host events, version ",$Version,"\n";
...
sub create_message_html {
  ...
  if (defined($o_addurl)) {
    $html_msg .= "<a href=\"$icinga2_cgiurl/monitoring/host/show?host=" . $o_hostname ."\">$o_hostname</a>";
  ...
  if (defined($o_addurl)) {
    $html_msg = $html_msg
              . "<a href=\"$icinga2_cgiurl/monitoring/list/hosts?hostgroup_name=" . $o_hostgroup_first ."\">$o_hostgroup</a>";
  ...
  if (defined($o_addurl)) {
    ...
    $html_msg .=  "<a href=\"$icinga2_cgiurl/monitoring/host/show?type=1&host=" . $o_hostname . "\">$o_hostoutput</a>\n";
    ...
    if ($o_format eq "graph" && $graph_type ne "gif") {
      ...
      $html_msg .= " <a href=\"$pnp4nagios_url/graph?host=" . $o_hostname ."&srv=_HOST_\">"
...
sub set_subject {
  ...
  my %lang =  (	'en' => "Icinga2: $o_notificationtype Host $o_hostname ($o_hostgroup) is $o_hoststate",
               	'pl' => "Icinga2: $o_notificationtype Host $o_hostname jest w statusie: $o_hoststate",
               	'de' => "Icinga2: $o_notificationtype System $o_hostname($o_hostgroup) ist $o_hoststate",
               	'jp' => "Icinga2: $o_notificationtype" . $b64_sub . "$o_hoststate",
               	'fr' => "Icinga2: $o_notificationtype" . $b64_sub . "$o_hoststate" );
